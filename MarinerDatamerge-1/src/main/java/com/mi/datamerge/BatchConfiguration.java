...
import com.mi.datamerge.service.JsonItemProcessor;
...
public class BatchConfiguration {
	...
	public FlatFileItemReader<ReportCsvDTO> csvReader() throws MalformedURLException {
		return new FlatFileItemReaderBuilder<ReportCsvDTO>().name("csvItemReader")
				.resource(new ClassPathResource(fileCsv)).linesToSkip(1).delimited()
				.names(new String[] { "clientAddress", "clientGuid", "requestTime", "serviceGuid", "retriesRequest",
	...
	public ItemReader<ReportXmlDTO> xmlReader() {
		...
		return new StaxEventItemReaderBuilder<ReportXmlDTO>().name("xmlReader").resource(new ClassPathResource(fileXml))
				.addFragmentRootElements("report").unmarshaller(reportMarshaller).build();
	}

	@Bean
	public JsonItemReader<ReportJsonDTO> jsonReader() {
		return new JsonItemReaderBuilder<ReportJsonDTO>().name("tradesJsonItemReader")
				.resource(new ClassPathResource(fileJson))
				.jsonObjectReader(new GsonJsonObjectReader<>(ReportJsonDTO.class)).build();
	}
	...
	@Bean
	public Job importDataJob(JobCompletionNotificationListener listener, Step stepCsv, Step stepXml, Step stepJson) {
		return jobBuilderFactory.get("importDataJob").incrementer(new RunIdIncrementer()).listener(listener)
				.flow(stepCsv).next(stepXml).next(stepJson).end().build();
	}

	@Bean
	public Step stepCsv(JdbcBatchItemWriter<ReportModel> writer) throws Exception {
		return stepBuilderFactory.get("stepCsv").<ReportCsvDTO, ReportModel>chunk(10).reader(csvReader())
				.processor(csvProcessor()).writer(writer).build();
	}

	@Bean
	public Step stepXml(JdbcBatchItemWriter<ReportModel> writer) throws Exception {
		return stepBuilderFactory.get("stepXml").<ReportXmlDTO, ReportModel>chunk(10).reader(xmlReader())
				.processor(xmlProcessor()).writer(writer).build();
	}

	@Bean
	public Step stepJson(JdbcBatchItemWriter<ReportModel> writer) throws Exception {
		return stepBuilderFactory.get("stepJson").<ReportJsonDTO, ReportModel>chunk(10).reader(jsonReader())
				.processor(jsonProcessor()).writer(writer).build();
	}
